...
func TestGetURLsFromHTML_InvalidBaseURL(t *testing.T) {
	inputBody := `<html><body><a href="/test">Test</a></body></html>`
	// Simulate a failed url.Parse by passing nil
	urls, err := getURLsFromHTML(inputBody, nil)
	if err == nil {
		t.Errorf("expected error for nil base URL, got nil")
	}
	if len(urls) != 0 {
